...
1. **`whitespace-pre-wrap` with no break rule** — `apps/web/src/components/item-detail.tsx:423`.
   `pre-wrap` wraps at *break opportunities* only; a 60-character token with no
...
2. **No decision at the container level** — `SheetContent` has `overflow-y-auto`
   ...
   when one axis is `auto`, so the sheet *accidentally* became a horizontal
...
This is the honest version of "let it scroll": a container that *chose* to be a
...
3. **Playwright scan** `apps/web/tests/overflow.spec.ts` — for each viewport in
   ...
   const bad = await page.evaluate(() => {
     ...
     if (root.scrollWidth > root.clientWidth + 1) out.push({ sel: ":root", by: root.scrollWidth - root.clientWidth });
     for (const el of document.querySelectorAll("*")) {
       ...
       if (ox === "clip" || ox === "hidden") { /* fall through: clipping is not an excuse */ }
       else if (ox === "auto" || ox === "scroll") { out.push({ sel: path(el), why: "undeclared scroller" }); continue; }
       if (el.scrollWidth > el.clientWidth + 1) out.push({ sel: path(el), by: el.scrollWidth - el.clientWidth });
     }
     return out;
   });
   expect(bad).toEqual([]);
   ...
   Note the two-sided check: an element that overflows is a failure *and* an
