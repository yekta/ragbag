...
//    and two persistence layers eventually disagree about which one won.
...
function SidebarProvider({
  ...
  // Helper to toggle the sidebar. Local edit (re-apply on a re-pull): upstream
  // names both updater params `open`, which shadows the `open` above and trips
  // `no-shadow`.
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          // Above the floating chrome in app.tsx (z-30), not below it. The
          // panel is a sheet over the page, so the page's own furniture belongs
          // underneath while it travels: that occlusion is what lets the
...
// Local edit (re-apply on a re-pull): safe-area insets on the two edge slots.
// Below `md` the sidebar is a flush, full-height Sheet, so on a notched phone
// its header sits under the status bar and its footer under the home
// indicator; shadcn's `p-2` has no notion of either. `max()` keeps the normal
// padding everywhere the inset is zero, so nothing changes on desktop.
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn(
        "flex flex-col gap-2 p-2 pt-[max(0.5rem,env(safe-area-inset-top))] pl-[max(0.5rem,env(safe-area-inset-left))]",
        className,
      )}
      {...props}
    />
...
function SidebarFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn(
        "flex flex-col gap-2 p-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pl-[max(0.5rem,env(safe-area-inset-left))]",
        className,
      )}
      {...props}
    />
...
function SidebarGroupAction({
  ...
  return useRender({
    ...
    props: mergeProps<"button">(
      {
        className: cn(
          "absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform group-data-[collapsible=icon]:hidden after:absolute after:-inset-2 hover:bg-hover hover:text-foreground active:bg-hover active:text-foreground focus-visible:ring-2 md:after:hidden [&>svg]:size-4 [&>svg]:shrink-0",
          className,
        ),
      },
      props,
...
const sidebarMenuButtonVariants = cva(
  ...
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md border-y-2 border-transparent bg-clip-padding p-2 text-left text-sm text-muted-foreground ring-sidebar-ring outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-hover hover:text-foreground focus-visible:ring-2 active:bg-hover active:text-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-hover data-open:hover:text-foreground data-active:bg-sidebar-accent data-active:text-sidebar-primary data-active:hover:bg-sidebar-accent-hover data-active:active:bg-sidebar-accent-hover data-[status=active]:bg-sidebar-accent data-[status=active]:text-sidebar-primary data-[status=active]:hover:bg-sidebar-accent-hover data-[status=active]:active:bg-sidebar-accent-hover [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
        default: "hover:bg-hover hover:text-foreground active:bg-hover active:text-foreground",
        outline:
          "bg-background shadow-[0_0_0_1px_var(--sidebar-border)] hover:bg-background-hover hover:text-foreground active:bg-background-hover active:text-foreground",
      },
      size: {
...
function SidebarMenuSubButton({
  ...
  return useRender({
    ...
    props: mergeProps<"a">(
      {
        className: cn(
          "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground ring-sidebar-ring outline-hidden group-data-[collapsible=icon]:hidden hover:bg-hover hover:text-foreground focus-visible:ring-2 active:bg-hover active:text-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[size=md]:text-sm data-[size=sm]:text-xs data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground data-active:hover:bg-sidebar-accent-hover data-active:active:bg-sidebar-accent-hover [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground",
          className,
        ),
      },
      props,
