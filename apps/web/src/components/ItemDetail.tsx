...
// lightbox (plan §10), plus the tag editor and pin/delete/retry actions.
    ...
    <Overlay onClose={close}>
      ...
      <div className="sticky top-0 z-10 flex items-center gap-2 border-b border-neutral-200 bg-white/95 px-5 py-3 backdrop-blur">
        ...
        <span className="ml-auto flex items-center gap-1">
          ...
          <button
            className={`rounded-lg p-2 hover:bg-neutral-100 ${item.pinned ? "text-amber-500" : "text-neutral-400"}`}
      ...
      <div className="space-y-5 px-5 py-5">
        ...
        {/* the user's text (note body / comment) */}
        <section>
          <SectionLabel>{item.kind === "note" ? "Note" : "Your comment"}</SectionLabel>
          {editing ? (
            <div>
              <textarea
            ...
            >
              {item.text ? (
                <p className="whitespace-pre-wrap leading-relaxed text-neutral-900">{item.text}</p>
              ) : (
