...
// 4. PUT the bytes to the presigned URL (no session — the URL is the bearer).
// In dev the URL points at the web origin (Vite proxy); talk to the API directly.
const uploadUrl = retry.uploadUrl.replace("http://localhost:5173", SERVER);
...
// 5. Same content, different client id → dedupe onto the canonical blob row,
// and no re-upload since the bytes exist now.
const second = await presign(newId());
if (second.blobId !== clientBlobId) fail(`dedupe returned ${second.blobId}, want ${clientBlobId}`);
if (second.uploadUrl !== null) fail("dedupe still asked for an upload");
console.log("OK   re-dump of identical bytes dedupes onto the existing blob (no upload)");
...
const got = await fetch(url.replace("http://localhost:5173", SERVER));
...
const tampered = await fetch(`${url.replace("http://localhost:5173", SERVER)}x`);
