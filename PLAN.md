...
- *Mobile (< md):* sidebar is off-canvas and opens as an overlay drawer over a scrim (the ItemDetail pattern, sliding from the left, keeping the floating-card inset). Floating hamburger top-left + search button top-right over the timeline (no ⌘K on touch); picking a filter / scrim tap / Esc closes. `sidebarOpen` is ephemeral view state.
