...
item
  ...
  user_id
  kind          'note' | 'todo' | 'address' | 'link' | 'image' | 'pdf' | 'file'
                # note/todo/address are the "user's own words" kinds and convert
                # into each other (item.setKind); ingestion may promote a note
                # it recognises as a todo/address, never anything else
  created_at, updated_at, deleted_at (soft delete)
  favorite      bool          # a filter view in the rail, NOT a pin-to-top
  completed_at  timestamptz   # todos: null = open, set = done
  text          user's message text (note/todo/address body, or a comment attached to a dump)
  url           for links
...
**Shell layout — floating sidebar + mobile (shipped 2026-08-12).** The composer set the visual language — floating cards on the `neutral-50` canvas — and the shell adopts it:
- *Floating sidebar (≥ md):* the rail drops its flush `border-r` panel look for a card matching the composer (inset margin, rounded, `border-neutral-200/90`, the same soft shadow).
- *Collapse:* toggle in the sidebar header + `⌘\`; animates the card away (width/translate/fade), a small floating button top-left over the timeline reopens it. Full collapse, not an icon mini-rail — the rail is filters-with-counts and tags, which icons can't carry, and search stays on ⌘K. `sidebarCollapsed` lives in the zustand view store and persists to localStorage, a deliberate exception to "view state never survives reload" (it's a device preference, like the remembered identity).
- *Mobile (< md):* sidebar is off-canvas and opens as a drawer over a dimmed overlay, sliding from the left — **flush and full-height, not a floating card** (the inset belongs to the desktop rail; on a phone the drawer is the screen). Floating hamburger top-left + search button top-right over the timeline (no ⌘K on touch); picking a filter / overlay tap / Esc closes. The drawer's open state is ephemeral and owned by the sidebar provider, not the view store.
- *Mobile polish riding along:* composer + drawer respect `env(safe-area-inset-*)`; composer textarea ≥16px (kills iOS focus-zoom) and no `autoFocus` on touch (it pops the keyboard on load); item detail full-screen below md; search overlay tightens padding, ⌘K hints hidden on touch; hover-only affordances (attachment remove, card actions) get tap-reachable equivalents.
- *Acceptance:* preview at 375/768/1280 — no horizontal scroll at 375, drawer + composer usable with the keyboard open, collapse survives reload, virtualization unaffected by the width transition.
- *As built (2026-08-12, superseded in part by the shadcn migration below):* the shell owned placement and rendered the Sidebar card twice; touch detection is a one-shot `matchMedia("(pointer: coarse)")` (`lib/touch.ts`); since hover actions have no touch equivalent, tapping a card body on coarse pointers opens the detail view; the viewport meta gained `viewport-fit=cover` (safe-area insets) and `interactive-widget=resizes-content` (keyboard pushes the floating composer up).

**shadcn/ui + the mint theme (shipped 2026-08-13).** The validated palette and the rules that hold it together live in the comment above the token block in `apps/web/src/index.css`. What changed:
- *Primitives:* shadcn/ui vendored into `src/components/ui/` (Radix + cva + `cn`). Adopted where it removes hand-rolled behaviour: `Sidebar` (below), `CommandDialog` for ⌘K (cmdk owns keyboard nav and selection — ~60 lines deleted), `Sheet` for item detail (focus trap, scroll lock, Esc), `DropdownMenu` for the composer's type picker (kills the click-away hack), `AlertDialog` replacing `window.confirm` on delete, plus Button/Badge/Alert/Textarea/ToggleGroup/Skeleton/Tooltip/Sonner. `@/` path alias throughout; the old `.js`-suffixed relative imports are gone.
- *File naming:* every component is kebab-case (`item-detail.tsx`, `tag-editor.tsx`, `app.tsx`), matching shadcn's own output. Exported identifiers stay PascalCase. Directory rule: `components/ui/*` is vendored, `components/*` is ours — that's what distinguishes our `components/sidebar.tsx` (the ragbag rail) from `components/ui/sidebar.tsx` (the primitive it's built on).
- *Theme:* the palette moved from hardcoded Tailwind classes to shadcn's token model in `index.css` — a **muted mint** built in OKLCH at hue 168–174. Three rules carry it. (1) The chroma budget is lopsided: surfaces and ink sit at 0.0015–0.012 (a light canvas of `#f1f3f2` against a neutral `#f2f2f2` — ~1/255 apart), while primary/ring/accent/kind tokens keep 0.02–0.18. Pushing surface chroma toward 0.015 is what made the first pass read as a green app. (2) **Everything carries the hue and nothing composites** — borders, rings and shadows included. No `bg-muted/40`, no `border: white/10%`, no `dark:bg-destructive/60`; soft fills are their own solid tokens (`--panel`, `--kind-*-soft`, `--destructive-soft`) and hovers are `-hover` tokens, because an alpha colour lands differently on the canvas, on a card, and inside the detail sheet. Shadows override Tailwind's `--shadow-*` scale with a mint tint (`57 84 75` light, `0 6 4` dark) at four strengths. The only translucent things left are the overlay and shadows — both physically translucent, both tinted; shadows bake their alpha into the token, and the overlay stays an opaque colour whose strength is a named `--opacity-overlay` modifier (`bg-overlay/overlay`). Verified by a rendered-DOM audit: zero compositing colours outside the overlay, zero neutral-black shadows. (3) **The canvas lands in the middle of its 8-bit bucket.** The browser paints `--background` through several paths on one screen — the fill propagated from `body`, an element repainting it, a composited layer, the `theme-color` chrome — and `oklch(0.19 0.005 174)` converted to `17.589, 20.530, 19.559`, every channel a hair from rounding either way. Two paths rounded it opposite ways and drew a full-width seam across the sign-in screen; both canvases are now tuned to land within 0.08 of an integer, same hexes, same ratios. Light **and** dark; every text pair clears WCAG AA and focus rings clear 3:1 (ratios in the `index.css` header). Beyond the shadcn set: `--warning`/`--success` (sync banner, sync dot, todo tick), seven `--kind-*` tokens replacing the per-kind Tailwind palette, and `--ai` for ingestion-generated surfaces. Theme choice (light/dark/system) is a device preference in the view store + localStorage, applied pre-paint by an inline script in `index.html`.
- *Sidebar:* `variant="floating" collapsible="offcanvas"`. The primitive applies the inset/rounding/shadow **only in its `md`+ branch** and renders a flush full-height `Sheet` below it, so "floating on desktop, not on mobile" falls out of the component rather than a media-query hack. The provider runs controlled from the view store.
- *Deliberate edits to vendored files* (re-apply if those components are ever re-pulled; each is commented in place): **every file in `ui/` had its alpha modifiers swept to opaque tokens** (see the note atop `ui/button.tsx`); `ui/sidebar.tsx` drops the `sidebar_state` cookie write (localStorage is the single source of truth), uses `⌘\` instead of `⌘B`, widths match the old `w-72` rail, the mobile Sheet carries `env(safe-area-inset-*)` padding and drops the Sheet's `shadow-lg`, and the floating variant is a flat `rounded-2xl` bordered card; `ui/command.tsx` gains a `commandProps` passthrough so the palette can set `shouldFilter={false}` (ranking is minisearch's); `ui/sonner.tsx` reads the theme from our store instead of `next-themes`.
- *Motion:* the generated timings were retuned — Sheet ran **500ms in / 300ms out on `ease-in-out`** (an S-curve barely moves for the first fifth of a half-second slide, which is what reads as slow) and the sidebar rail on `ease-linear`. Now two tokens in `index.css`, `--ease-enter` (decelerate hard) and `--ease-exit` (accelerate away), with sheets at 200/150ms, dialogs at 150/100ms, the rail at 200ms, and the sheet overlay timed to its own panel instead of drifting. `prefers-reduced-motion` collapses it all. Two traps: the animated panels need `transition-none` (the `duration-*` utilities also set `transition-duration`, and CSS defaults `transition-property` to `all`, so every colour on the panel silently gained a transition), and `shadow-float` has to be a `--shadow-*` theme key rather than a `@utility` — a utility writing `box-shadow:` clobbers any `ring-*` on the same element. Separately, **item-detail had no exit animation at all**: closing navigated away, unmounting the component before Radix could play it (the panel vanished in one frame, 23ms, while the state-driven mobile drawer slid out over ~240ms). It now holds local `open` state, defers the route change, and paints the last known item during the exit so deleting doesn't flash the spinner on the way out. Production-bundle measurement: drawer click → settled in 217–265ms.
- *Canvas strip behind the composer:* cards used to slide under the floating composer and vanish against a hard edge. A plain `bg-background` strip now sits **inside the composer's own wrapper**, bottom-aligned and `calc(var(--composer-inset) + 1rem)` tall, behind the card and `pointer-events-none`. It started as a gradient fading to 75%, but the ramp read as haze over the bottom of the timeline; solid is cleaner and keeps the palette free of a third alpha exception. Its own top edge never shows: in the middle it's hidden behind the card, and either side it's background over background (the timeline column is inset further than the composer card). Scoping it to the composer is the point: sized off the shell column instead it becomes a fraction of the whole page. Sized off the *gap* rather than the card, because the card grows with its draft while the distance to the bottom is constant; `--composer-inset` is declared once and feeds both the wrapper's padding and the strip's height.
- *Left alone on purpose:* the timeline's scroll container is still a plain div — `@tanstack/react-virtual` measures `getScrollElement`, and `ScrollArea`'s nested Radix viewport would break it. Same for the composer's drop overlay, which must stay `pointer-events-none` or the window-level drop handlers die.
- *Verified 2026-08-13* by headless Playwright at 375/768/1280 in both themes: rail floating at 1280/768 (inset 8px, 16px radius), drawer flush at 375 (x=0, y=0, full height, 0 radius), no horizontal scroll at 375, collapse survives reload with no cookie written, virtualized scroller intact, detail sheet / delete dialog / ⌘K palette all functional, no console or page errors.
...
*As built: queue worker in the API process (`FOR UPDATE SKIP LOCKED` claim, `LISTEN/NOTIFY` wake, stale-job reclaim, backoff; `INGEST_WORKER=false` hands over to a worker instance per §11). Link extraction via linkedom + Readability with an article snapshot to blob storage; video links are metadata-only (`isVideoUrl` in `shared`); PDF text via pdfjs; images via one `gpt-5.6-luna` vision call; enrichment prompts carry the owner's topic vocabulary for convergence. `ai_usage` prices every call for auditing; there is no spend cap (the old rolling-24h budget silently skipped enrichment, which is indistinguishable from a broken pipeline — removed 2026-08-14). `item_chunk` holds chunks + a generated tsvector, with `embedding vector(1536)` + HNSW when pgvector exists — M7's Tier-2 groundwork, verified end to end against pgvector 0.8.1 (embeddings written at the right dimensionality, cosine ANN retrieval semantically sensible, HNSW index valid for the cosine opclass). A server that gains pgvector after migrating heals itself on the next boot: `ensureVectorColumn()` creates the extension where permitted, adds the column, and builds the index.*
*Deviations: (1) **notes get jobs too** — they skip extraction but still earn AI tags/summary, so `needsIngest` is now unconditional; (2) failures are typed — `WaitingError` (a blob still uploading from an offline device) reschedules without burning an attempt, `PermanentError` (no PDF text layer, refused host, a 4xx from the vision API on unreadable image bytes) fails immediately, everything else retries up to 4 times; (3) fetching user-supplied URLs added a **baseline SSRF guard** (DNS-resolved private/link-local/CGNAT addresses refused, redirects re-checked per hop, byte cap, timeout) — not in the plan text, but this endpoint fetches arbitrary URLs on a user's behalf; (4) OCR for scanned PDFs is deliberately still deferred, matching §7's "defer with status failed + manual retry". Scanned-PDF rasterization and the Batch API backfill path remain open. Acceptance test: `apps/server/scripts/ingest-proof.mts` — covers every kind (note, link, video-shaped link, PDF, text file, image, plus a refused host and corrupt image bytes), asserting extraction/indexing without a key and the AI stages when the server has one. It renders its own PNG and PDF rather than carrying binary fixtures. The image assertion checks the *structure* our code produces (a description plus `ocr_text` round-tripped as its own field) and only logs transcription drift: the same 5x7-bitmap image reads back as "RASBEG"/"RAGBRS"/"RAGERS" across runs, so any similarity threshold would fail on model variance instead of our bugs.*

**Post-M4 — Favorites, todos, addresses** ✅ *done 2026-08-12*
Pins became **favorites**: `item.pinned` renamed to `item.favorite` (migration `0004`), and the pinned strip above the timeline is gone — favorites are a filter view in the rail, so the archive stays one chronological stream. Two new kinds joined the sidebar: **todos** (`completed_at`, a checkbox on the card and in the detail view, and a rail badge that counts *open* ones) and **addresses** (stored as typed, with Open-in-Maps/Copy actions built from a maps *search* URL — no geocoding, no API key). Three ways in, in order of authority: the composer's type picker, the markers people already type (`todo:`, `- [ ]`, `address:` — stripped on capture, `parseTextCapture` in `shared`), and enrichment's new `suggestedKind`, which promotes a *note* only. That promotion is ingestion's one write to a user-authored row and it is fenced in SQL (`where kind = 'note'`), so an explicit choice — composer or `item.setKind` — always wins, including across a re-run. Text kinds convert freely into each other; a link can't become an image. Acceptance: `ingest-proof.mts` grew a todo, an address and a task-shaped note (asserting the fence holds and logging the model's verdict, like the OCR assertion).
