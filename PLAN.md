...
- *Deliberate edits to vendored files* (re-apply if those components are ever re-pulled; each is commented in place): **every file in `ui/` had its alpha modifiers swept to opaque tokens** (see the note atop `ui/button.tsx`); `ui/sidebar.tsx` drops the `sidebar_state` cookie write (localStorage is the single source of truth), uses `⌘\` instead of `⌘B`, widths match the old `w-72` rail, the mobile Sheet carries `env(safe-area-inset-*)` padding, and the floating variant uses the app's `shadow-float`/`rounded-2xl`; `ui/command.tsx` gains a `commandProps` passthrough so the palette can set `shouldFilter={false}` (ranking is minisearch's); `ui/sonner.tsx` reads the theme from our store instead of `next-themes`.
