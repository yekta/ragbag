...
- *Canvas fade behind the composer:* cards used to slide under the floating composer and vanish against a hard edge. A `fade-to-canvas` gradient now sits over the bottom half of the timeline column — solid `--background` at the very bottom, half strength at the midpoint, gone at the top — layered between the cards (`z-10`) and the composer card (`z-20`), and `pointer-events-none` so it never eats scroll. The upper half of the ramp is the part that matters: stopping the gradient at 50% would have put a 0%→50% step across the middle of the timeline, a worse seam than the one it fixes. Built with `color-mix` so the stops stay derived from `--background` per theme rather than restated.
