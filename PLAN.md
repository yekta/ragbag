...
- *Canvas fade behind the composer:* cards used to slide under the floating composer and vanish against a hard edge. A `fade-to-canvas` gradient now sits **inside the composer's own wrapper**, bottom-aligned and `calc(var(--composer-inset) + 1rem)` tall — solid `--background` at the bottom, half strength at the midpoint, gone at the top — behind the card and `pointer-events-none`. Scoping it to the composer is the point: sized off the shell column instead it becomes a fraction of the whole page and washes out half the timeline. Sized off the *gap* rather than the card, because the card grows with its draft while the distance to the bottom is constant; `--composer-inset` is declared once and feeds both the wrapper's padding and the fade's height. The upper part of the ramp matters too — stopping at 50% opacity would leave a visible step at the gradient's top edge. Built with `color-mix` so the stops stay derived from `--background` per theme rather than restated.
