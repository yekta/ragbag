...
- *Mobile (< md):* sidebar is off-canvas and opens as an overlay drawer over a scrim, sliding from the left — **flush and full-height, not a floating card** (the inset belongs to the desktop rail; on a phone the drawer is the screen). Floating hamburger top-left + search button top-right over the timeline (no ⌘K on touch); picking a filter / scrim tap / Esc closes. The drawer's open state is ephemeral and owned by the sidebar provider, not the view store.
