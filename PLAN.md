...
- *Canvas fade behind the composer:* cards used to slide under the floating composer and vanish against a hard edge. A `fade-to-canvas` gradient now sits **inside the composer's own wrapper**, bottom-aligned and `calc(var(--composer-inset) + 1rem)` tall — solid `--background` at the bottom ramping to 75% at the top — behind the card and `pointer-events-none`. It needn't reach transparent: the top edge is hidden behind the card, and beside the card it's background over background. Scoping it to the composer is the point: sized off the shell column instead it becomes a fraction of the whole page and washes out half the timeline. Sized off the *gap* rather than the card, because the card grows with its draft while the distance to the bottom is constant; `--composer-inset` is declared once and feeds both the wrapper's padding and the fade's height. Built with `color-mix` so the stops stay derived from `--background` per theme rather than restated.
